...
def is_trajectory_clean(coord):
    global trajectory_is_here
    if coord[0] == trajectory_is_here[0] and coord[1] == trajectory_is_here[1]:
        print('trajectory_is_here!')
        return True
    else:
        return False
...
def find_a_way(maze, coord, possible_ways):
    ...
    len_y, len_x = len(maze), len(maze[0])
    global path_to_exit, current_path, trajectory_is_found
    ...
    for direction in possible_ways:
        if is_trajectory_clean(step(coord, direction)) or trajectory_is_found:
            trajectory_is_found = True
...
with open("maze_done.txt", "w") as txt_file:
    for line in maze:
